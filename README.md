...
- **七牛 Kodo 数据源**：分别填入七牛 AccessKey 和 SecretKey 即可远程直连私有对象存储空间；支持一键创建私有 `calendarmark` 空间、选择已有空间，侧栏左下角显示账号本月的多维额度用量。
...
选择七牛作为当前数据源时：按月读取远端、保存直接写远端、删除直接删除远端对象，行为与 Notion 直连一致。配置完成后，侧栏左下角（数据源入口上方）会显示账号本月标准存储、GET 请求、PUT/DELETE 请求、CDN 回源流量和外网流出流量；前四项会对照七牛官方免费额度显示进度条，外网流出流量按量计费；统计接口有约 5 分钟延迟。切到 Notion 或本地存储时，左下角不会显示七牛卡片，也不会发起七牛统计请求。
